...
namespace HybridCLR.Editor
{
    ...
    internal class HTEditorCache
    {
        ...
        const string il2cppPlus = "il2cpp_plus";
        const string hybridclr = "hybridclr";
        public static string GetZipInnerFolder(EFILE_NAME nameType, InstallVersion version)
        {
            switch (nameType)
            {
                case EFILE_NAME.IL2CPP:
                    return $"{il2cppPlus}-{version.il2cppTag}/libil2cpp";
                case EFILE_NAME.IL2CPP_BRANCH:
                    return $"{il2cppPlus}-{version.il2cppBranch}/libil2cpp";
                case EFILE_NAME.HUATUO_MAIN:
                    return $"{hybridclr}-main/huatuo";
                case EFILE_NAME.HUATUO:
                    return $"{hybridclr}-{version.huatuoTag}/huatuo";
            }
            return "error param";
        }
        public string GetZipPath(EFILE_NAME nameType, InstallVersion version)
        {
            ...
            {
                case EFILE_NAME.HUATUO_MAIN:
                    zipFileName = $"{hybridclr}-{version.huatuoTag}";
                    break;
                case EFILE_NAME.HUATUO:
                    zipFileName = $"{hybridclr}-{version.huatuoTag}";
                    break;
                case EFILE_NAME.IL2CPP_BRANCH:
                    zipFileName = $"{il2cppPlus}-{version.il2cppBranch}-{version.il2cppTag}";
                    break;
                case EFILE_NAME.IL2CPP:
                    zipFileName = $"{il2cppPlus}-{version.il2cppTag}";
                    break;
                default:
                    throw new Exception($"no support file type{nameof(nameType)}");
            }
